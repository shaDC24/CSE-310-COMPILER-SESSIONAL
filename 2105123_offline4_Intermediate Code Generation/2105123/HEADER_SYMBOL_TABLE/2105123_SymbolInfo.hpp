...
class SymbolInfo {
 private:
  ...
  ExtraSymbolInfo *extrainfo;
    ...
    int stack_offset;
    bool is_global;
    int FunctionStackOffset;
    vector<pair<string,int>>FunctionStackOffset_parameter;
...
param *getParam(int index) {
    ...
    void addFunctionParameter(string type, string name) {
        if (funcInfo != nullptr) {
            ...
            cout<<"Inserting "<<getsymbolName()<<" e "<<type<<endl;
        }
    }
    ...
    bool isParameter(string s)
    {
        return this->funcInfo->isParameter(s);
    }
    void setFunctionStackOffset(int st)
    {
        this->FunctionStackOffset=st;
    }
    int getFunctionStackOffset()
    {
      return this->FunctionStackOffset;
    }
    void setFunctionStackOffset_parameter(string name,int st)
    {
        this->FunctionStackOffset_parameter.push_back({name,st});
        
    }
    int getFunctionStackOffset_parameter(string name)
    {
        for(int i=0;i<FunctionStackOffset_parameter.size();i++)
        {
            cout<<FunctionStackOffset_parameter[i].first<<"  "<<FunctionStackOffset_parameter[i].second<<endl;
        }
        for(int i=0;i<FunctionStackOffset_parameter.size();i++)
        {
            if(FunctionStackOffset_parameter[i].first==name)
            {
                return FunctionStackOffset_parameter[i].second;
            }
        }
        return -1;
    }
...
/*void printSymbolInfo(const SymbolInfo* symbol) {
  ...
  int getStackOffset()
  {
    //cout<<"Got ---------"<<this->symbolName<<" Offset--------"<<this->stack_offset<<endl;
    return this->stack_offset;
  }
  void setStackOffset(int so)
  {
    this->stack_offset=so;
    cout<<"Got ---------"<<this->symbolName<<" Offset--------"<<this->stack_offset<<endl;
    
  }
  void set_is_global(bool g=false)
  {
    this->is_global=g;
  }
  bool get_is_global()
